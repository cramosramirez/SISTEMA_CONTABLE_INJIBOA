...
namespace SistemaContable.UI.Forms.NotaRemision
{
    public partial class frmDespacho : Form
    {
        ...
        private void CargarNRExistente(int IdTraslado)
        {
            ...
            finally
            {
                ...
                if (string.IsNullOrWhiteSpace(txtKilogramos.Text))
                {
                    ...
                    { btOrdenDespacho.Enabled = true; txtOrdenDespacho.Enabled = true; }
                    else { btOrdenDespacho.Enabled = false; txtOrdenDespacho.Enabled = false; }
                }
                else
                {
        ...
        #region viewOrdenDespacho
        private void CargarODxistente(int IdOD)
        {
            try
            {
                Cursor = Cursors.WaitCursor;
                ...
                if (Convert.ToBoolean(r["YA_ASIGNADA"]))
                {
                    XtraMessageBox.Show(
                        r["ASIGNADA"].ToString(),
                        "Orden de Despacho ya relacionada",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Warning);
        ...
        private void CargarODDTxistente(int IdOD)
        {
            try
            {
                DataTable dt2 = _dal.EjecutarConsulta("[EORDEN_DESPACHO].VIEW_NOTAREMISION_DET",
                    new
                    {
        ...
        {
            if (!int.TryParse(txtOrdenDespacho.Text, out int ordenDespacho))
            {
                DevExpress.XtraEditors.XtraMessageBox.Show(
                    "Debe ingresar un número de Orden de Despacho válido.",
                    "Validación",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Warning);

                txtOrdenDespacho.Focus();
                return;
            }
