...
{
    public partial class frmDocumentoCompraRemesa : Form
    {
        ...
        private void frmDocumentoCompra_Load(object sender, EventArgs e)
        {
            ...
            FormHelper.RegistrarBusqueda(
                ...
                new BusquedaConfig
                {
                    StoredProcedure = "[EPROVEEDOR].[SP_ENTIDAD]",
                    Accion = "BUSCAR",
            ...
            if (IdCompraRemesa > 0)
            {
                CargarCompraRemesaExistente(IdCompraRemesa);
        ...
        private void CargarCompraRemesaExistente(int IdCompraRemesa)
        {
            try
            {
                Cursor = Cursors.WaitCursor;

                DataTable dt = _dal.EjecutarConsulta("[EPROVEEDOR].[SP_COMPRA_REMESA]",
                    new
                    {
                        ACCION = "OBTENER",
                        ID_COMPRA_REMESA = IdCompraRemesa
                    });
                ...
                txtUnico.Text = r["UNICO"].ToString();
                ...
                txtIDSIAP.Text = AsString(r["ORDEN"]);
                ...
                int idTipoImportacion = ToInt(r["ID_TIPO_IMPORTACION"]);
                ...
                // Independientes
                cbxTIPO_IMPORTACION.SelectedValue = idTipoImportacion;
        ...
        {
            var num = _dal.ObtenerNumeracionPrevia("CTR", null);   // correlativo exclusivo de Compra Exterior
            if (num == null)
            {
                MessageBox.Show(
                    "No existe numeración activa para Compra por Remesa (CTR).\n" +
            ...
            if (num.IdTipoDte != 35 ||
                !string.Equals(num.Abreviatura, "CTR", StringComparison.OrdinalIgnoreCase))
            {
                MessageBox.Show(
                    "La numeración de Compra Exterior debe corresponder a " +
                    "ID_TIPO_DTE = 35 y TIPODTE = CTR.",
        ...
        {
            CargarCombo("[EPROVEEDOR].[SP_TIPO_DTE]", "BUSCAR_COMPRAS_TRANFERENCIA", "ID_TIPO_DTE", "ABREVIATURA", cbxTIPO_DTE);
            CargarCombo("[EPROVEEDOR].[SP_SUCURSAL]", "BUSCAR", "ID_SUCURSAL", "NOMBRE", cbxSUCURSAL);
        }
        ...
        private void CargarTipoImportacion()
        {
        ...
        {
            DataTable dt = _dal.EjecutarConsulta("[EPROVEEDOR].[SP_COMPRA_CLASIFICACION]",
                new
                {
                    ACCION = "COMBO",
        ...
        {
            DataTable dt = _dal.EjecutarConsulta("[EPROVEEDOR].[SP_COMPRA_SECTOR]",
                new
                {
                    ACCION = "COMBO",
                    ID_TIPO_OPERA = idTipoOpera,
        ...
        {
            DataTable dt = _dal.EjecutarConsulta("[EPROVEEDOR].[SP_COMPRA_TIPO_COSTO]",
                new
                {
                    ACCION = "COMBO",
                    ID_TIPO_OPERA = idTipoOpera,
        ...
        {
            DataTable dt = _dal.EjecutarConsulta("[EPROVEEDOR].[SP_TIPO_RENTA]",
                new
                {
                    ACCION = "COMBO"
                });
        ...
        private void txtPROVEEDOR_Leave(object sender, EventArgs e)
        {
            ...
            try
            {
                var dt = _dal.EjecutarConsulta("[EPROVEEDOR].[SP_ENTIDAD]", new
                {
        ...
        private void CargarComboFiltrado(int idClasifica)
        {
            try
            {
                var dt = _dal.EjecutarConsulta("[EPROVEEDOR].[SP_COMPRA_TIPO_COSTO]", new
                {
        ...
        private void RecalcularTotales()
        {
            ...
            decimal fovial = 0; // ObtenerDecimal(txtFOVIAL);
            decimal contrans = 0; // ObtenerDecimal(txtCONTRANS);
        ...
        {
            if (!(cbxTIPO_DTE.SelectedItem is DataRowView fila))
                return string.Empty;

            object valor = fila["TIPO_DTE"];
            return valor == null || valor == DBNull.Value
                ? string.Empty
                : valor.ToString().Trim().ToUpperInvariant();
        }

        private void SeleccionarRentaPorTipoDocumento()
        {
            string codigoDocumento = ObtenerCodigoTipoDocumentoSeleccionado();
            string codigoRenta;
            ...
            if (!(cbxTIPO_RENTA.DataSource is DataTable tabla)) return;

            DataRow seleccion = null;
            foreach (DataRow fila in tabla.Rows)
            {
                if (!string.Equals(Convert.ToString(fila["CODIGO"]).Trim(), codigoRenta,
            ...
            if (seleccion != null)
                cbxTIPO_RENTA.SelectedValue = Convert.ToInt32(seleccion["ID_TIPO_RENTA"]);
        }
        ...
        {
            if (IdCompraRemesa == 0 && !string.IsNullOrWhiteSpace(txtCOD_GENERACION.Text.Trim()) && !string.IsNullOrWhiteSpace(txtNUM_CONTROL.Text.Trim()))
            {
        ...
        private bool ValidarCampos()
        {
            ...
            // Cascada de clasificación
            if (!TieneSeleccion(cbxTIPO_IMPORTACION)) { AvisoValidacion(cbxTIPO_IMPORTACION, "tipo de importación"); return false; }
        ...
        private void GuardarDocumento()
        {
            try
            {
                ...
                var parametros = new
                {
                    ...
                    ORDEN = NullIfEmpty(txtIDSIAP.Text),
                    ...
                    UNICO = txtUnico.Text,
                    IDSIAP = txtIDSIAP.Text
                };

                DataTable dt = _dal.EjecutarConsulta("[EPROVEEDOR].[SP_COMPRA_REMESA]", parametros);
                ...
                IdCompraRemesa = Convert.ToInt32(row["ID_GENERADO"]);
        ...
        private void InicializarHelperMinisterioHacienda()
        {
            ...
            _mhHelper = new MinisterioHaciendaHelper(
                ...
                actualizarFOVIAL: (texto) => txtFOVIAL = texto,
                actualizarCOTRANS: (texto) => txtCONTRANS = texto,
                actualizarIVAR: (texto) => txtIVAR = texto,
        ...
        {
            using (var frm = new frmDocumentoCompraProvision("EPROVEEDOR"))
            {
                frm.IdCcfCompra = IdCompraRemesa;
                frm.ShowDialog(this);
            }
        }
        ...
        private void btnConsultaCheque_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt = _dal.EjecutarConsulta("[EPROVEEDOR].[SP_COMPRA_EXTERIOR]",
                    new
                    {
                        ACCION = "OBTENER_CHEQUE",
                        ID_COMPRA_EXTERIOR = IdCompraRemesa
                    });
        ...
        Process.Start(new ProcessStartInfo(
            "https://aduana2.mh.gob.sv/SIAP/free/declarationPaid.do")
        {
